...
public class PlannerService {
    ...
    public List<MateriaPlanoDTO> gerarPlanoDeEstudos(UUID usuarioId, int totalCardsPorDia) {
        ...
        //retornar somente as matérias e conteúdos com os conteúdos que tem o número de cards alocados maior que 0
        return plano.stream()
                .filter(m -> m.cardsAlocados() > 0)
                .map(m -> new MateriaPlanoDTO(
                        m.materiaId(),
                        m.materiaNome(),
                        m.taxaErroMateria(),
                        m.cardsAlocados(),
                        m.conteudos().stream()
                                .filter(c -> c.cardsAlocados() > 0)
                                .toList()
                ))
                .toList();
    }
